...
export const Footer = () => {
  return <footer className="pb-[20px] border-t border-[var(--color-border)] ">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between items-center gap-8">
        ...
        <nav className="flex flex-col md:flex-row gap-8 md:gap-16 items-center ">
          {navItems.map((item)=>(
            ...
            className="uppercase text-xs tracking-widest font-bold text-gray-400">{item.name}</a>
          ))}
        </nav>
      </div>
        
      <div className="mt-16 flex flex-col md:justify-between md:flex-row-reverse items-center gap-8">
