  ...
  return <section id="pricing">
    <div className="container">
      <SectionBorder borderTop>
        <SectionContent>
          <h2 className="text-3xl md:text-4xl lg:text-5xl text-center font-semibold text-gray-200 leading-tight">Flexible plans for every need</h2>

          <div className="mt-12 flex flex-col gap-8 md:flex-row lg:items-start ">
            {pricingTiers.map((tier)=>(
              <div key={tier.title} className={twMerge("border border-[var(--color-border)] rounded-3xl px-6 py-12 max-w-sm mx-auto flex-1", tier.className)}>
                <h3 className={twMerge("font-semibold text-4xl", tier.color === 'violet' && 'text-violet-400', 
                  tier.color === 'amber' && 'text-amber-300',  tier.color === 'teal' && 'text-teal-300'
                 )}>{tier.title}</h3>

                <p className="text-gray-400 text-xs mt-4 tracking-wide">{tier.description}</p>
                ...
                <ul className="flex flex-col gap-4 mt-8">
                  {tier.features.map((feature)=>(
                    <li key={feature} className="border-t border-[var(--color-border)] pt-4 flex gap-4">
                      <FontAwesomeIcon icon={faCheckCircle} className="size-6 text-violet-400 flex-shrink-0" /><span className="font-medium text-sm">{feature}</span></li>
                  ))}
                </ul>
              </div>
            ))}
