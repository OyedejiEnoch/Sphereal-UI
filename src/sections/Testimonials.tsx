        ...
        <SectionContent>
          ...
          <motion.div layout className="border-gradient rounded-3xl px-6 md:px-10 lg:px-16 lg:py-24 py-16 relative flex flex-col md:flex-row gap-12 md:mx-10 lg:mx-20">
            ...
            {testimonials.map((testimonial, index)=>(
              ...
              {testimonialIndex === index && (
                ...
                <cite className="not-italic lg:text-right">
                  <Image src={testimonial.image} alt={testimonial.name} className="rounded-xl size-20 max-w-none"/>
            ...
            <div className="flex gap-2  md:flex-col ">
              {testimonials.map((testimonial, index)=>(
                <div onClick={()=>setTestimonialIndex(index)} key={testimonial.name} className="size-6 relative isolate inline-flex items-center justify-center cursor-pointer">
                  {testimonialIndex === index && 
                  <motion.div className="absolute border-gradient size-full rounded-full z-10 "
                  layoutId="testimonal-dot"
                  ></motion.div>
                  }
